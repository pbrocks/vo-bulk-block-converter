...
class VO_Post_Table extends WP_List_Table {
	...
	public static function set_args_for_query() {
		...
		} elseif ( ! isset( $_REQUEST['vo_from_date'] ) ) {
		...
		} elseif ( ! empty( $_REQUEST['vo_to_date'] ) ) {
			$to = date( 'Y-m-d 23:59:59', strtotime( $_REQUEST['vo_to_date'] ) );
		...
		if ( $which == 'top' ) {
			...
			<div class="alignleft actions bulkactions">
				<select name="vo_get_errors">
					...
					<option value="1" <?php echo ( ! empty( $_REQUEST['vo_get_errors'] ) ? 'selected = "selected"' : '' ); ?>>HTML Block Warnings</option>
				</select>
				...
				<input type="text" name="vo_from_date" value="<?php echo isset( $_REQUEST['vo_from_date'] ) ? $_REQUEST['vo_from_date'] : self::$default_from_date; ?>" placeholder="From (YYYY-MM-DD)"/>
				<input type="text" name="vo_to_date" value="<?php echo isset( $_REQUEST['vo_to_date'] ) ? $_REQUEST['vo_to_date'] : ''; ?>" placeholder="To (YYYY-MM-DD)"/>
				<?php submit_button( __( 'Filter', VO_DOMAIN ), 'action', 'vo_filter_btn', false ); ?>
				<?php if ( empty( $_REQUEST['vo_get_errors'] ) ) : ?>
					<?php submit_button( sprintf( __( 'Convert Current Filter (%d)', VO_DOMAIN ), self::$total_items ), 'primary', 'vo_convert_filter', false, array( 'data-nonce' => $bulk_nonce ) ); ?>
				<?php endif; ?>
			</div>
			<?php
		}
